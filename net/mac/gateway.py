...
#				if pkt != tmp:
#					if abs(tmp.freq - pkt.freq) <= 120 and (tmp.bw == 500 or pkt.freq == 500):
#						tmp.collision = 1
#					elif abs(tmp.freq - pkt.freq) <= 60 and (tmp.bw == 250 or pkt.freq == 250):
#						tmp.collision = 1
#					elif abs(tmp.freq - pkt.freq) <= 30:
#						tmp.collision = 1

			tmp.ok						= 0 if tmp.lost or tmp.collision else tmp.ok
		if pkt.ok:
#			self.tm_                     = app.G2S(self.gm_)
#			self.tm                      = app.G2S(self.gm) # np.mean([self.T[i][j-7] for i in self.params.freqSet for j in self.params.sfSet]) #
			return 1
#		self.tm_                     = app.G2S(self.gm_)
#		self.tm                      = app.G2S(self.gm_) # np.mean([self.T[i][j-7] for i in self.params.freqSet for j in self.params.sfSet]) #
		return 0
