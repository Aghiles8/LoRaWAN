import numpy 				as np
from   app           import app
from   net.mac.packet 		import myPacket, myPacketAck
import queue
...
#		self.HH						= list()
	...
	def reload(self):
#		mmber             = np.loadtxt(self.params.topopath+'fcm/0_fcm_i.csv', dtype='int'  , delimiter=',', skiprows=0)[:,self.edapp]
#		self.setActions   = [v for i, v in enumerate(self.setActions) if i not in np.argwhere(mmber == 0).flatten().tolist()] 
#		self.actions			= len(self.setActions)
#		self.params.server.card[self.edapp] = self.actions
#		self.HH						= list()
		self.PP						= queue.Queue(20)
		self.H						= {tmp:myPacket for tmp in range(self.params.nrBS)}
		self.P						= {tmp:myPacket for tmp in range(self.params.nrBS)}
		...
		for i in range(3):
			self.policy[i]	= [1/self.actions for x in range(self.actions)]    #np.random.rand(self.actions) #	= [self.policy[i][x]/sum(self.policy[i]) for x in range(self.actions)] # [ for x in range(self.actions)]   
		self.app					= self.edapp
