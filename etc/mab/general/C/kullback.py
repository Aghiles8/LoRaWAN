...
def klGauss(x, y, sig2 = 0.25):
    """caution modified""" """Kullback-Leibler divergence for Gaussian distributions."""
    return (x-y)*(x-y)/(2*sig2)

def klucb(x, d, div, upperbound, lowerbound=-float('inf'), precision=1e-6):
    """The generic klUCB index computation.    Input args.: x, d, div, upperbound, lowerbound=-float('inf'), precision=1e-6,    where div is the KL divergence to be used.    """
    l = max(x, lowerbound)
    u = upperbound
    while u-l>precision:
        m = (l+u)/2
...
def klucbGauss(x, d, sig2=1., precision=0.):
    """klUCB index computation for Gaussian distributions.    Note that it does not require any search."""
...
def maxEV(p, V, klMax):
    """Maximize expectation of V wrt. q st. KL(p,q) < klMax.    Input args.: p, V, klMax.    Reference: Section 3.2 of [Filippi, Cappé & Garivier - Allerton, 2011].    """
    Uq = np.zeros(len(p))
    Kb = p>0.
    K = ~Kb
    if any(K):
...
def reseqp(p, V, klMax):
    """Solve f(reseqp(p, V, klMax)) = klMax using Newton method.    Note: This is a subroutine of maxEV.    Reference: Eq. (4) in Section 3.2 of [Filippi, Cappé & Garivier - Allerton, 2011].    """
    mV = max(V)
    l = mV+0.1
    tol = 1e-4
    if mV<min(V)+tol:
